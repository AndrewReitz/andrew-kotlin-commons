...
class ResultsKtTest {
    ...
    @JsName("expect_should_return_value_or_error_if_not_available")
    @Test
    fun `expect should return value or error if not available`() {
        assertFailsWith<IllegalStateException> {
            Result.failure(Exception("Some Error"))
                .expect("Could not load value.")
        }

        val expectedResultString = "Yay!"
        val result = Result.success(expectedResultString).expect("Should not happen")
        assertEquals(actual = result, expected = expectedResultString)
    }
}
